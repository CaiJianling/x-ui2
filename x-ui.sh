...
ssl_cert_issue() {
    echo -E ""
    LOGD "******使用说明******"
    LOGI "该脚本将使用Acme脚本通过Cloudflare API申请证书,使用时需保证:"
...
ssl_cert_issue_aliyun() {
    echo -E ""
    LOGD "******使用说明******"
    LOGI "该脚本将使用Acme脚本通过阿里云API申请证书,使用时需保证:"
    LOGI "1.知晓阿里云AccessKey ID"
    LOGI "2.知晓阿里云AccessKey Secret"
    LOGI "3.域名已通过阿里云进行解析到当前服务器"
    LOGI "4.该脚本申请证书默认安装路径为/root/cert目录"
    confirm "我已确认以上内容[y/n]" "y"
    if [ $? -eq 0 ]; then
        cd ~
        ...
        LOGD "请设置域名:"
        read -p "Input your domain here:" Ali_Domain
        LOGD "你的域名设置为:${Ali_Domain}"
        LOGD "请设置阿里云AccessKey ID:"
        read -p "Input your Aliyun AccessKey ID here:" Ali_Key
        LOGD "你的阿里云AccessKey ID为:${Ali_Key}"
        LOGD "请设置阿里云AccessKey Secret:"
        read -p "Input your Aliyun AccessKey Secret here:" Ali_Secret
        LOGD "你的阿里云AccessKey Secret为:${Ali_Secret}"
        ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt
        if [ $? -ne 0 ]; then
            LOGE "修改默认CA为Lets'Encrypt失败,脚本退出"
            exit 1
        ...
        ~/.acme.sh/acme.sh --issue --dns dns_ali -d ${Ali_Domain} -d *.${Ali_Domain} --log
        if [ $? -ne 0 ]; then
            LOGE "证书签发失败,脚本退出"
            exit 1
        else
        ...
        ~/.acme.sh/acme.sh --installcert -d ${Ali_Domain} -d *.${Ali_Domain} --ca-file /root/cert/ca.cer \
        --cert-file /root/cert/${Ali_Domain}.cer --key-file /root/cert/${Ali_Domain}.key \
        --fullchain-file /root/cert/fullchain.cer
        if [ $? -ne 0 ]; then
            LOGE "证书安装失败,脚本退出"
            exit 1
...
————————————————
  ...
  ${green}16.${plain} 一键申请SSL证书(Cloudflare DNS验证)
  ${green}17.${plain} 一键申请SSL证书(阿里云DNS验证)
 "
    show_status
